<script lang="ts">
  ...
  function split_input_schemas(field_info_maps: Record<string, FieldInfo>) {
    ...
    [
      ...
      _all_fields,
      _required_fields,
      _required_fields_with_defaults,
      _optional_fields,
    ];
    return _components;
  }
  ...
  $: current_values = objectMap(fields_info, (v) =>
    v != null
      ? v.field_schema.default == "__not_set__" ||
        v.field_schema.default == "__no_value__"
        ? null
        : v.field_schema.default
      : null
  );
            ...
            <div class="input-panel-container">
              {#each Object.entries(required_fields) as [field_name, field_info]}
                <div class="input-panel">
                  <svelte:component
                    this={field_components[field_name]}
                    {field_info}
                    current_value={current_values[field_name]}
                    on:input_changed={handle_input_changes}
                  />
                </div>
              {/each}
            </div>
