<script lang="ts">
  ...
  function handle_selection_update(new_aliases: string[]) {
    ...
    if (
      new_aliases.length == 0 ||
      (new_aliases.length == 1 && new_aliases[0] == null)
    ) {
      ...
      throw Error(
        "Invalid selection, only 1 element can be selected at any one time."
      );
    } else {
    ...
    if (new_alias == null) {
      ...
      if (current_value == null || !current_value.aliases.includes(new_alias)) {
        ...
        if (new_value == null) {
          ...
          if (
            current_value == null ||
            new_value.value_id != current_value.value_id
          ) {
